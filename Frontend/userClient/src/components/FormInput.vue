...
<template>
	<div class="relative h-[8rem] flex justify-start items-end text-body text-screenBlack">
		<div class="h-[50%] relative w-full border-b-4 border-b-screenBlack has-[:focus]:border-b-primary transition-all duration-100">
			<input 
				:type="localType"
				:value="modelValue"
				...
				autocomplete="true"
			>
			<label :for="randomId" class="px-[2rem] font-heading absolute w-full h-full top-0 left-0 justify-start items-center pointer-events-none peer-focus:-translate-y-[100%] peer-focus:text-primary transition-all duration-100 peer-[:not(:placeholder-shown)]:-translate-y-[100%] peer-[:autofill]:-translate-y-[100%]">
				{{label}}
			</label>
			<button v-if="type == 'password'" @click="toggleInputType" class="absolute h-full aspect-square right-[2rem] top-0 flex justify-center items-center fill-screenBlack lg:hover:fill-primary group">
				...
				<div class="absolute w-full h-full top-0 left-0 pointer-events-none">
					<div class="relative w-full h-full">
						<div class="absolute w-full h-full top-0 left-0 origin-center scale-[1.1] flex justify-center items-center z-[10] rotate-45">
							<transition
